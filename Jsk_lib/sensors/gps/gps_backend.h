...
public:
  ...
  GPS_Backend(UART_HandleTypeDef *huart, ros::NodeHandle* nh)
  {
  ...
  {
    bool is_valid = rx_buf_.pop(pop_value);
    return is_valid;
  }
...
/*
 ...
 //uart rx part
  static RingBufferFiFo<uint8_t, RING_BUFFER_SIZE> rx_buf_;
  static uint8_t rx_value_[GPS_RX_SIZE];
  static uint16_t rx_size_;

protected:

  UART_HandleTypeDef *huart_;
  ros::NodeHandle* nh_;
  ros::Subscriber2<std_msgs::UInt8, GPS_Backend>* gps_config_sub_;
  ...
  void baseInit (UART_HandleTypeDef* huart, ros::NodeHandle* nh)
  {
    huart_ =huart;
    nh_ = nh;
    gps_config_sub_ = new ros::Subscriber2<std_msgs::UInt8, GPS_Backend> ("/gps_config_cmd", &GPS_Backend::gpsConfigCallback, this );
